...
public function reserve(Request $request, Voyage $voyage, EntityManagerInterface $entityManager): Response
{
    ...
    $depart    = $voyage->getDateDepart()  ? $voyage->getDateDepart()->format('d/m/Y')  : 'N/A';
    $arrivee   = $voyage->getDateArrivee() ? $voyage->getDateArrivee()->format('d/m/Y') : 'N/A';
    $prix      = $voyage->getPrix() ? number_format($voyage->getPrix(), 2, ',', ' ') . ' €' : 'N/A';
    $userName  = $user->getFirstName() . ' ' . $user->getLastName();
    $msg = "✈️ *VoyageLoisir — Nouvelle réservation !*\n\n"
         . "👤 Client : *{$userName}*\n"
         . "📍 Destination : *{$destNom}*\n"
         . "📅 Départ : {$depart}\n"
         . "📅 Retour : {$arrivee}\n"
...
public function checkout(Voyage $voyage): Response
{
    ...
    $session = \Stripe\Checkout\Session::create([
        ...
        'mode'        => 'payment',
        'success_url' => $this->generateUrl('app_payment_success', ['id' => $voyage->getId()], \Symfony\Component\Routing\Generator\UrlGeneratorInterface::ABSOLUTE_URL),
        'cancel_url'  => $this->generateUrl('app_payment_cancel',  ['id' => $voyage->getId()], \Symfony\Component\Routing\Generator\UrlGeneratorInterface::ABSOLUTE_URL),
    ]);
